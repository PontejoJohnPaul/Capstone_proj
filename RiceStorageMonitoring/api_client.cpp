...
void checkSystemStatus()
{
    ...
    String url = String(SERVER_URL) + "get_live_monitoring.php";
    ...
    if (httpCode > 0)
    {
        ...
        if (error)
        {
            Serial.println("JSON Parse Failed! (live monitoring)");
            http.end();
            return;
        }
        ...
        //--------------------------------------------------
        // Find the WORST (most severe) risk_status among all
        // sensors that currently have one (i.e. status "online").
        // Same ranking used server-side: SAFE=0, WARNING=1, DANGER=2
        //--------------------------------------------------
        int worstRank = -1;
        const char* worstStatus = nullptr;

        for (JsonObject sensor : sensors)
        {
            const char* risk = sensor["risk_status"];
            ...
            if (strcmp(risk, "DANGER") == 0)
                rank = 2;
            else if (strcmp(risk, "WARNING") == 0)
                rank = 1;
            else if (strcmp(risk, "SAFE") == 0)
                rank = 0;
            else
                continue;
        ...
        Serial.print("System Risk Status : ");
        Serial.println(worstStatus != nullptr ? worstStatus : "OFFLINE");

        setLedStatus(worstStatus);
        setBuzzerStatus(worstStatus);
    }
    else
    {
        Serial.print("HTTP Error : ");
