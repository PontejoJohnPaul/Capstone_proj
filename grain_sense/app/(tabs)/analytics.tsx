...
import {
  ...
  TAB_TO_RANGE,
  TabKey,
  buildBatchResultsQuery,
  confirmSensorAction,
} from '@/constants/analyticsTypes';
import { monitorStyles, paginationStyles, styles } from '@/styles/analytics.styles';
...
export default function AnalyticsScreen() {
  ...
  const [batchResultsLoading, setBatchResultsLoading] = useState(true);
  const [batchFilters, setBatchFilters] = useState<BatchFilters>(DEFAULT_BATCH_FILTERS);
  ...
  const loadBatchResults = useCallback(async () => {
    ...
    try {
      ...
      if (!farmerId) return;
      const query = buildBatchResultsQuery(farmerId, batchPage, batchFilters);
      const res = await fetch(`${API_BASE_URL}/get_batch_results.php?${query}`);
      const data = await res.json();
      if (data.success) {
      ...
      console.error('GrainSense Analytics: failed to load batch results ->', error);
    } finally {
      setBatchResultsLoading(false);
    }
  }, [batchPage, batchFilters]);
  ...
  // Changing any filter jumps back to page 1 so results stay consistent.
  function handleBatchFiltersChange(next: BatchFilters) {
    setBatchFilters(next);
    setBatchPage(1);
  }

  function handleResetBatchFilters() {
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {sensors.length === 0 ? (
          ...
          <View style={monitorStyles.grid}>
            {sensors.map((sensor) => (
              <View key={sensor.sensor_id} style={monitorStyles.gridItem}>
                <SensorMonitorCard
                  sensor={sensor}
        ...
        <Text style={[monitorStyles.sectionTitle, { marginTop: 8 }]}>Batch Results</Text>

        <BatchFilterBar
          filters={batchFilters}
          onChange={handleBatchFiltersChange}
          onReset={handleResetBatchFilters}
        />
          ...
          <>
            ...
            {batchTotalPages > 1 && (
              <View style={paginationStyles.row}>
                <TouchableOpacity
                  style={[paginationStyles.btn, batchPage <= 1 && paginationStyles.btnDisabled]}
                  disabled={batchPage <= 1}
                  onPress={() => setBatchPage((p) => Math.max(1, p - 1))}
                >
                  <Ionicons
                    name="chevron-back"
                    size={14}
                    color={batchPage <= 1 ? '#BBBBBB' : GREEN_MID}
                  />
                  <Text
                    style={[
                      paginationStyles.btnText,
                      batchPage <= 1 && paginationStyles.btnTextDisabled,
                    ]}
                  >
                    Prev
                  </Text>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    batchPage >= batchTotalPages && paginationStyles.btnDisabled,
                  ]}
                  disabled={batchPage >= batchTotalPages}
                  onPress={() => setBatchPage((p) => Math.min(batchTotalPages, p + 1))}
                >
                  <Text
                    style={[
                      paginationStyles.btnText,
                      batchPage >= batchTotalPages && paginationStyles.btnTextDisabled,
                    ]}
                  >
                    Next
                  </Text>
                  <Ionicons
                    ...
                    size={14}
                    color={batchPage >= batchTotalPages ? '#BBBBBB' : GREEN_MID}
                  />
                </TouchableOpacity>
              </View>
            )}
